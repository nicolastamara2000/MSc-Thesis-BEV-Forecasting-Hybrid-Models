...
8. [Defense Preparation — Potential Questions and Answers](#8-defense-preparation--potential-questions-and-answers)
...
## 8. Defense Preparation — Potential Questions and Answers

### Q1: Why not use a pure deep learning approach (e.g., Transformer, N-BEATS) instead of the hybrid framework?

**A**: With only 144 observations total (108 for training), pure deep learning models would be severely data-starved. Transformers and N-BEATS require thousands or millions of data points to learn meaningful patterns. The hybrid approach injects strong domain knowledge (S-curve adoption dynamics) through the diffusion model, requiring only 3–6 parameters, and lets the neural model focus on the much simpler task of modeling near-stationary residuals. This is a principled application of the bias-variance tradeoff: we accept the bias of assuming S-curve dynamics (well-justified by adoption theory) to dramatically reduce variance. Empirically, the LSTM results (which struggles even on residuals) demonstrate that neural models alone are insufficient with this sample size.

### Q2: How did you select the SARIMAX order (1,0,1)(1,0,0,12)?

**A**: The order was guided by the ACF and PACF plots of the diffusion residuals. The ACF showed significant correlation at lag 1 (suggesting MA(1)) and at lag 12 (seasonal). The PACF showed a sharp cutoff after lag 1 (suggesting AR(1)). No differencing was needed (d=0, D=0) because the diffusion model already removes the trend, and the ADF test on residuals confirmed near-stationarity. The seasonal component (1,0,0,12) captures the annual cycle with a single seasonal AR parameter. This parsimonious specification avoids overfitting on the relatively short training series.

### Q3: Why does the LSTM perform so poorly?

**A**: The LSTM's poor performance stems from several factors:
- **Small training set**: With lookback=24 and ~108 training points, there are only ~84 training windows. LSTMs have many parameters (the embedding + LSTM + head totals several thousand) and need much more data to train effectively.
- **Autoregressive error accumulation**: Over a 24-month horizon, each prediction error feeds into the next step, causing errors to compound.
- **Overfitting**: With 64 hidden units and limited data, the LSTM likely memorizes training patterns that don't generalize.
The TCN, with its simpler architecture (32 hidden channels) and parallel processing, is more parameter-efficient and better suited to this data regime.

### Q4: What is the advantage of the Dynamic Bass model over the standard Bass model?

**A**: The standard Bass assumes a fixed market potential `m`. In reality, the addressable EV market grows over time as charging infrastructure expands, new vehicle models launch, and government policies evolve. The Dynamic Bass replaces `m` with a logistic function `M(t)` that grows from `M_min` to `M_max`, adding only 3 extra parameters. For Norway, which is transitioning from rapid growth to saturation, this is particularly important: the Dynamic Bass achieves notably better results because the time-varying market potential captures the decelerating growth rate. For Italy, still in early exponential growth, the standard Bass already captures the trend well, so the Dynamic extension offers less marginal benefit (and may even overfit slightly).

### Q5: Why use sMAPE instead of MAPE?

**A**: Standard MAPE (Mean Absolute Percentage Error) has two problems: (1) it is undefined when actual values are zero, and (2) it is asymmetric — it penalizes over-predictions more than under-predictions. sMAPE addresses both issues by using the average of actual and predicted values in the denominator: `2*|y - y_hat| / (|y| + |y_hat|)`. This gives a bounded, symmetric percentage error. For BEV data where magnitudes vary enormously (33 to 34,000+), a percentage-based metric complements MAE and RMSE by providing scale-independent assessment.

### Q6: How do you handle the exponential growth in the data? Isn't fitting on raw values problematic?

**A**: Yes, this is precisely why the `log1p` transform is applied. On the raw scale, the loss function is dominated by recent observations (which are 100–1000x larger than early observations), causing the diffusion model to underfit the early growth phase. The `log1p` transform equalizes the contribution of all time periods, and the results clearly show that log-scale models outperform raw-scale models by a wide margin (e.g., sMAPE of 4.45% vs. 72–94% for Italy). The `log1p` (rather than `log`) is used to handle potential zero or near-zero values gracefully.

### Q7: What happens if the diffusion model is wrong about the market potential?

**A**: This is a known limitation of diffusion models — the market potential `m` (or `M_max` for Dynamic Bass) is estimated from historical data and may not reflect future reality. However, the hybrid framework provides a safety net: even if the diffusion model's long-run estimate is imperfect, the residual model can partially compensate for systematic deviations in the test period. Additionally, the Dynamic Bass model's time-varying potential provides more flexibility than a fixed `m`. For true production use, the models should be re-estimated periodically as new data arrives, which naturally updates the market potential estimates.

### Q8: Why a 24-month forecast horizon?

**A**: A 24-month (2-year) horizon is the sweet spot for practical relevance and methodological challenge:
- **Practical value**: Government and industry planning cycles typically operate on 1–3 year horizons (infrastructure investment, production planning, policy evaluation).
- **Methodological challenge**: Short horizons (1–3 months) are too easy for most models; very long horizons (5+ years) make quantitative evaluation unreliable because too many external factors change.
- **Sufficient test data**: 24 months provides enough test observations to compute meaningful error statistics while still leaving 108 months for training.

### Q9: Why didn't you use cross-validation?

**A**: Standard k-fold cross-validation is inappropriate for time series because it violates temporal ordering — future data would "leak" into training folds. Time series cross-validation (e.g., expanding or sliding window) could theoretically be applied, but with only 144 total observations, further splitting would leave too few points for either training or evaluation. The single temporal split (108 train / 24 test) was chosen as the most reliable evaluation strategy given the data constraints.

### Q10: How robust are the neural network results given the random initialization?

**A**: Seeds are fixed (`torch.manual_seed(42)`, `np.random.seed(42)`) for reproducibility, so the reported results are deterministic given the same code and data. However, neural networks are known to be sensitive to initialization, and results could vary with different seeds. The TCN's simpler architecture and the fact that it operates on near-stationary residuals (rather than raw non-stationary data) make it more robust to initialization than a deep LSTM. Ideally, future work should report results averaged over multiple seeds with confidence intervals.

### Q11: What are the main limitations of this work?

**A**:
1. **Sample size**: 144 monthly observations is small for neural network training. More years of data or daily resolution would improve neural model performance.
2. **Only two countries**: While Italy and Norway represent different adoption stages, generalization to other markets is not validated.
3. **No exogenous variables**: The models don't incorporate fuel prices, subsidies, charging infrastructure density, or macroeconomic indicators, which could improve forecasts.
4. **Single train/test split**: A single evaluation window may not capture model performance under different economic conditions.
5. **No uncertainty quantification**: Point forecasts are provided without confidence intervals, which are important for decision-making.

### Q12: What would you do differently if you could extend this work?

**A**:
1. **Add exogenous variables** (fuel prices, government subsidies, charging point density) to the residual model via SARIMAX(X) or as additional neural network features.
2. **Ensemble methods**: Combine predictions from multiple model variants (weighted averaging or stacking).
3. **Probabilistic forecasting**: Use quantile regression or Monte Carlo dropout to produce prediction intervals.
4. **More countries**: Validate the framework on diverse markets (e.g., Germany, China, USA) to demonstrate broader applicability.
5. **Daily or weekly data**: Higher-frequency data would give the neural models much more training signal.

### Q13: Can you explain the Multi-Horizon TCN architecture more precisely?

**A**: The MH-TCN has two conceptual stages:
1. **Encoding**: The past 36 residuals are processed by 4 stacked causal convolutional blocks (dilations 1,2,4,8; kernel 3; 32 channels). Causal padding ensures no future information leaks. The output at the last time step is a 32-dimensional **context vector** summarizing the residual history.
2. **Decoding**: This context vector is replicated 24 times (one per future month). Each replica is concatenated with that month's sin/cos features (2 dimensions), giving a 34-dimensional input per step. A shared MLP (`Linear(34, 64) → ReLU → Linear(64, 1)`) maps each to a single residual prediction. All 24 predictions are output simultaneously and trained with MSE loss against all 24 true future residuals.

This is a **direct multi-output** strategy: the model learns to predict the full trajectory at once, avoiding the sequential error accumulation of autoregressive approaches.

### Q14: How does the framework handle the COVID-19 pandemic period (2020-2021)?

**A**: The COVID-19 pandemic falls within the training period (2014-2022). The diffusion model, being a smooth S-curve, naturally smooths over the COVID dip and recovery, treating it as part of the long-run trend. The residual model then captures the pandemic-induced deviations as short-term shocks. This decomposition is actually beneficial: the diffusion model isn't distorted by the pandemic (since it fits the overall S-curve shape), while the residual model learns that temporary disruptions can occur and revert. The outlier detection step (rolling z-score) helps identify such shock months, though in practice only August 2025 in Italy was flagged with |z| > 2.5.

### Q15: Why not use Prophet, DeepAR, or other off-the-shelf forecasting tools?

**A**: While tools like Prophet or DeepAR are excellent for general-purpose forecasting, this thesis specifically investigates the value of **domain-specific diffusion modeling** for technology adoption. The core research question is whether incorporating adoption theory (Bass model) as a structural prior improves forecasts compared to purely data-driven approaches. Using off-the-shelf tools would bypass this question entirely. Additionally, the custom implementations allow precise control over the hybrid architecture and direct comparison of different diffusion variants, which wouldn't be possible with pre-built tools.

---

## Accuracy Verification Checklist

The following facts have been verified against the notebook outputs:

- [x] Data spans Jan 2014 – Dec 2025 for both countries (144 observations each)
- [x] Both datasets have 3 columns (Ano, Mes, BEV registrations) with no missing values
- [x] Italy mean registrations ≈ 2,766 per month; range 33 – 15,304
- [x] ADF test on Italy raw: stat=0.1041, p=0.9663 (non-stationary) — confirmed
- [x] ADF test on Italy log1p diff(1): stat=-3.5645, p=0.006 (stationary) — confirmed
- [x] Bass params Italy (log1p): p=0.000118, q=0.0569, m=256,123 — confirmed
- [x] Bass params Norway (log1p): p=0.00121, q=0.0324, m=801,552 — confirmed
- [x] Dynamic Bass Italy: p=5.2e-5, q=0.080, M_min=153,644, M_max=305,697 — confirmed
- [x] SARIMAX order is (1,0,1)(1,0,0,12) with no trend — confirmed
- [x] LSTM: hidden=64, month_emb=8, lookback=24 — confirmed
- [x] TCN: hidden=32, levels=4, kernel=3, dropout=0.15, lookback=36 — confirmed
- [x] MH-TCN: hidden=32, levels=4, kernel=3, dropout=0.15, horizon=24, lookback=36 — confirmed
- [x] Bass + MH-TCN Italy log: MAE=0.375, RMSE=0.444, sMAPE=4.45% — confirmed
- [x] DynamicBass + MH-TCN Norway log: MAE=0.312, RMSE=0.479, sMAPE=3.51% — confirmed
- [x] Logistic model Italy: K=226,102, r=0.095, t0=94.8 — confirmed
- [x] Logistic model Norway: K=1,256,958, r=0.035, t0=117.6 — confirmed
- [x] LSTM consistently underperforms other residual models — confirmed
- [x] Log-scale models outperform raw-scale models across all configurations — confirmed
- [x] Train end is December 2022, test horizon is 24 months — confirmed
- [x] Seeds fixed at 42 for neural models — confirmed
- [x] Outlier detected: Italy Aug 2025 with z=-2.53 — confirmed
